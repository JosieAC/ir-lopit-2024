...
plot_marker_profiles <- function(obj,
  ...
  exprs_df <- exprs(obj)
  f_df <- fData(obj) %>% dplyr::select(!!sym(as.String(fcol)))
  ...
  p <- ggplot(exprs_df, aes(sample, abundance, colour=!!sym(as.String(fcol)))) +
  ...
  if(plot_all){
    ...
    if(!missing(group_by)){
      p <- p + aes(group=interaction(!!sym(as.String(group_by)), id))
    }
  } else{
    if(!missing(group_by)){
      if(length(alpha)==1){
        p <- p +
          stat_summary(aes(group=interaction(!!sym(as.String(fcol)), !!sym(as.String(group_by)))),
                       geom="line", fun.y=mean, size=1, alpha=alpha)
      } else{
        p <- p +
          stat_summary(aes(group=interaction(!!sym(as.String(fcol)), !!sym(as.String(group_by))),
                           alpha=!!sym(as.String(fcol))),
                       geom="line", fun.y=mean, size=1, alpha=alpha)
      }
    }
    else{
      if(length(alpha)==1){
        p <- p +
          stat_summary(aes(group=!!sym(as.String(fcol))),
                       geom="line", fun.y=mean, size=1, alpha=alpha)
      } else{
        p <- p +
          stat_summary(aes(group=!!sym(as.String(fcol)), alpha=!!sym(as.String(fcol))),
                       geom="line", fun.y=mean, size=1, alpha=alpha)
      }
    }
  }
  ...
  if(facet_by=='features'){
    ...
    p <- p +
      facet_wrap(vars(!!sym(as.String(fcol))))
  }
