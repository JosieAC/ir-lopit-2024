...
plotLabelQuantPSM <- function(obj, log=F, print=TRUE){
  tmp_df <- data.frame(exprs(obj))
  colnames(tmp_df) <- pData(obj)$Sample_name
  tmp_df[tmp_df==""] <- NA
  tmp_df <- melt(tmp_df, id.vars=NULL)
...
agg_to_peptide_mod <- function(obj, gb=NULL, mod_col="Modifications", seq_col="Sequence", method =sum){
  
  if(missing(gb)){
    gb <- paste(fData(obj)[[seq_col]], fData(obj)[[mod_col]])
  }

  pep_agg <- combineFeatures(obj, groupBy=gb, method =function(x) myAggFunction(x, fun))
...
restrictPepPerProtein <- function(peptide_obj, min_peptides, plot=TRUE){
  ...
  retain_mask <- n_pep_per_prot %>%
    merge(fData(peptide_obj)[,c('Annotated.Sequence', 'Master.Protein.Accessions'), drop=FALSE],
          by='Master.Protein.Accessions') %>%
    mutate(retain=n>=min_peptides) %>%
    select(Annotated.Sequence, sample, retain) %>%
    spread(key=sample, value=retain) %>%
    tibble::column_to_rownames('Annotated.Sequence')
